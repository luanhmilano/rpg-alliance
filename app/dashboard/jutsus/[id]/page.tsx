              ...
              {jutsu.effects.length ? (
                <div className="space-y-2">
                  {jutsu.effects.map((effect) => (
                    <div key={effect.id} className="rounded-md border p-3 space-y-1">
                      <p className="text-muted-foreground">
                        <span className="font-medium text-foreground">Escopo:</span>{" "}
                        {TARGET_SCOPE_LABEL[effect.targetScope] ?? effect.targetScope}
                      </p>
                      <p className="text-muted-foreground">
                        <span className="font-medium text-foreground">Atributo:</span> {effect.affectedAttribute}
                      </p>
                      <p className="text-muted-foreground">
                        <span className="font-medium text-foreground">Tipo:</span>{" "}
                        {EFFECT_KIND_LABEL[effect.effectKind] ?? effect.effectKind}
                      </p>
                      <p className="text-muted-foreground">
                        <span className="font-medium text-foreground">Operacao:</span>{" "}
                        {EFFECT_OPERATION_LABEL[effect.operation] ?? effect.operation}
                      </p>
                      <p className="text-muted-foreground">
                        <span className="font-medium text-foreground">Ordem de execucao:</span>{" "}
                        {effect.executionOrder ?? "N/D"}
                      </p>
                      <p className="text-muted-foreground whitespace-pre-line">
                        <span className="font-medium text-foreground">Valor:</span> {formatEffectValue(effect.value)}
                      </p>
                    </div>
                  ))}
                </div>
